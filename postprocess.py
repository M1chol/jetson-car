...
parser.add_argument("--timestep", "-t", action="store_const", help="set timestep, default 0.1", default=0.1)
parser.add_argument("--source", "-s", action="store_const", help="source folder where out.txt is located", default="results/temp/out.txt")
...
pipeline = dt.DataPipeline(
        ...
        [dt.WholeFileInterpolator(), dt.WholeFilePathCalculator(timestep=args.timestep, wheelRadius=wh, diffLength=df)],
...
if IS_STREAMLIT:
    ...
    chart = alt.Chart(pipeline.result)

    # Get min/max values for both axes
    min_val = min(pipeline.result['pos_x'].min(),
                pipeline.result['pos_y'].min())
    max_val = max(pipeline.result['pos_x'].max(),
                pipeline.result['pos_y'].max())
